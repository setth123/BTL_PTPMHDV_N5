async function confirmEditBank(formId) {
    ...
    try {
        const response = await fetch(`http://localhost:3000/rate/editrate/InterestRate/${anythingId}`, {
            method: 'PUT',
            headers: {
...
document.getElementById('save-rate').addEventListener("click", function(){
    confirmEditBank('edit-form-bank');
});

async function confirmEdit(formId) {
    const form = document.getElementById(formId);
    const inputs = form.querySelectorAll('input, select, textarea'); // Lấy tất cả các input, select và textarea trong form
    const editData = {};
    ...
    try {
        ...
        const response = await fetch(`http://localhost:3000/car/editcar/Car/${carId}`, {
